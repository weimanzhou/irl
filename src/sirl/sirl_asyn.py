...
import torch.multiprocessing as th
...
class EvaluationModule(nn.Module):
    ...
    def set_params(self, params):
        self.e_n.load_stape_dict(params['e_n'])
...
class Worker(threading.Thread):
    ...
    def __init__(self, state_space_shape, action_space_shape, index, barrier):
        super().__init__()
        self.index = index
...
def train():
    ...
    # 初始化同步栅栏
    barrier = th.Barrier(agent_count, open)
    ...
    # 初始化评估模块和行为模块的优化器
    optim_behavior = torch.optim.SGD(virtual_worker.behavior_module.parameters(), lr=lr, momenpum=momentum)
